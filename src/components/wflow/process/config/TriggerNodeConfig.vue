    ...
    <el-form label-position="top" label-width="90px">
      ...
      <div v-if="config.type === 'WEBHOOK'">
        <el-form-item label="请求地址" prop="text">
          <el-input placeholder="请输入URL地址" size="default" v-model="config.http.url" >
        ...
        <el-form-item label="Header请求参数" prop="text">
          <div slot="label">
            ...
            <span>参数类型 - </span>
            <el-radio-group size="default" style="margin: 0 5px;" v-model="config.http.contentType">
      ...
      <div v-else-if="config.type === 'EMAIL'">
        <el-form-item label="邮件主题" prop="text">
          <el-input placeholder="请输入邮件主题" size="default" v-model="config.email.subject" />
        </el-form-item>
    ...
    setup(props, { emit }) {
      ...
      const addItem = (items: any) => {
        if (items.length > 0 && (items[items.length - 1].name.trim() === '' || items[items.length - 1].value.trim() === '')) {
          proxy.$message.warning("请完善之前项后在添加")
          return;
        }
