...
// 引入表单子控件的管理类
import formItemManage from './../js/formItemManage'
// 引入组件需要的属性
import { baseFormProps } from './../js/formItemMeta'
 ...
 * ** change 事件的 myChange
 */
const dateManage = (value:any, mySubmit:any, controlType:any, controlId:any) => {
  ...
  const myChange = (_val:any) => {
    ...
    switch (controlType) {
      ...
      case 113: // 第几周。直接去dom找。没发现更好的方法
        setTimeout(() => {
          re = (document.getElementsByName('c' + controlId)[0] as HTMLInputElement).value
          console.log('周1：', re)
