<template>
  <div class="main">
    <div class="content">
      ...
      <opened></opened>
      <div class="table">
        <DiyTable :style="{ width: '100%' }" ref="diyTable" :hasTabs="true" :tableName="'我的应用'" :hasTitle="true"
          :hasTableHead="true" :tableData="state.ownProductList" :options="options" @handleUpdate="handleUpdate"
          @selectionChange="selectionChange" :tableHead="state.tableHead">
          <template #slot-tabs>
            <div class="table-tabs">
              <el-menu :default-active="tableActiveIndex" class="el-menu-demo" mode="horizontal" @select="handleSelect">
          ...
          <template #buttons>
            ...
            <el-button class="btn-check" @click="goCreate()" type="primary" link>创建</el-button>
            ...
            <el-dropdown trigger="click">
              ...
              <template #dropdown>
                <el-dropdown-menu>
                  ...
                  " link type="primary" @click="handleCommand('own', 'putaway', scope.row)">上架</el-dropdown-item>
                  <el-dropdown-item link type="primary" v-if="scope.row.belongId == store.workspaceData.id"
                    ...
                    <el-dropdown trigger="hover" placement="top-end">
                      ...
                      <template #dropdown>
                        <el-dropdown-menu style="padding-left: 10px; min-width: 100px">
                          <el-dropdown-item v-for="resource in scope.row.resourcesList" :key="resource.formId"
                            @click="enterProcess(resource)">{{ resource.business }}</el-dropdown-item>
                        </el-dropdown-menu>
                      </template>
                    </el-dropdown>
                  </el-dropdown-item>
    ...
    <el-dialog v-if="dialogType.cohortVisible" v-model="dialogType.cohortVisible" custom-class="share-dialog"
      title="应用分配" width="1000px" draggable :close-on-click-modal="false">
      <ShareComponent dialogType="1" @closeDialog="closeDialog('cohortVisible', false)" :info="selectProductItem">
      </ShareComponent>
    </el-dialog>
    <el-dialog v-if="dialogType.shareVisible" v-model="dialogType.shareVisible" custom-class="share-dialog" title="应用共享"
      width="1000px" draggable :close-on-click-modal="false">
      <ShareComponent dialogType="2" :type="store.workspaceData.type" @closeDialog="closeDialog('shareVisible', false)"
        :info="selectProductItem"></ShareComponent>
    </el-dialog>
