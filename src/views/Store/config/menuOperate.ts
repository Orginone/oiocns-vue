...
/** 获取组织菜单 */
const getTeamMenu = () => {
  console.log('orgCtrl.user.companys',orgCtrl.user.companys);
  const children: MenuItemType[] = [];
  for (const company of orgCtrl.user.companys) {
    children.push(
      createMenu(company, [
        ...buildDepartmentTree(company.departments),
        ...buildGroupTree(company.groups.filter((i) => i.belongId == company.id)),
...
export const loadBrowserMenu = () => {
  console.log('getUserMenu(), ...getTeamMenu()',getUserMenu(), ...getTeamMenu());
  return {
    key: 'disk',
    label: '数据',
    itemType: 'Tab',
