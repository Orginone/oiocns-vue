...
<script setup lang="ts">
  ...
  const typePD: any = ref(3)
  const application = new Application('4', 4)
  interface Tree {
    id: string
    label: string
    data?: any
  ...
  const submitAll = async () => {
    ...
    // })
    var record = state.identitysData.map(item=>{return {id:item.id , name:item.name} }) 
